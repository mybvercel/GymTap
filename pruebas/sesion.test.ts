...
prueba("la primera lectura ya registra la serie 1", () => {
  ...
  assert.equal(r.sesion.bloques[0].series.length, 1);
  assert.equal(r.sesion.bloques[0].series[0].n, 1);
});

prueba("la segunda lectura registra la serie 2", () => {
  ...
  assert.equal(b.sesion.bloques[0].series.length, 2);
  assert.equal(b.sesion.bloques[0].series[1].n, 2);
  assert.equal(b.sesion.bloques[0].series[1].peso, 40);
});
...
prueba("pasada la ventana, la lectura sí registra", () => {
  ...
  assert.equal(c.sesion.bloques[0].series.length, 3);
});
...
prueba("el descanso se mide solo entre serie y serie", () => {
  ...
  const serie3 = c.sesion.bloques[0].series[2];
  assert.equal(serie3.descansoPrevio, 90);
...
prueba("borrar la última serie renumera las que quedan", () => {
  ...
  assert.equal(s.bloques[0].series.length, 2);
  assert.deepEqual(s.bloques[0].series.map((x) => x.n), [1, 2]);
});
...
prueba("el volumen es peso por reps sumado", () => {
  ...
  assert.equal(volumen(s.bloques[0].series), 40 * 10 + 40 * 10 + 50 * 8);
});
...
prueba("cerrar un bloque le impide seguir recibiendo series", () => {
  ...
  assert.equal(r.sesion.bloques[1].series.length, 1);
});
